...
static NeighboursDicts BuildCoordNeighboursDicts(const unordered_map<string, Sphere::Point>& stop_points,
  ...
  for (const auto& company : companies) {
    const auto company_point = Sphere::DeserializePoint(company.address().coords());
    for (const auto& nearby_stop : company.nearby_stops()) {
      const auto stop_point = stop_points.at(nearby_stop.name());
      add_points(company_point, stop_point);
    }
  }
...
static vector<Sphere::Point> ExtractPoints(
    const unordered_map<string, Sphere::Point>& stop_points,
    const YellowPages::CompaniesProto& companies
) {
  vector<Sphere::Point> points;
  points.reserve(stop_points.size() + companies.size());
  for (const auto& [_, point] : stop_points) {
    points.push_back(point);
  }
  for (const auto& company : companies) {
    points.push_back(Sphere::DeserializePoint(company.address().coords()));
  }
  return points;
}
...
static PointsMaps ComputeObjectsCoordsByGrid(
    const Descriptions::StopsDict& stops_dict,
    const Descriptions::BusesDict& buses_dict,
    const YellowPages::CompaniesProto& companies,
    const RenderSettings& render_settings
) {
  const auto stop_points = ComputeInterpolatedStopsGeoCoords(stops_dict, buses_dict);

  const auto [neighbour_lats, neighbour_lons] = BuildCoordNeighboursDicts(stop_points, buses_dict, companies);

  CoordsCompressor compressor(ExtractPoints(stop_points, companies));
  ...
  PointsMaps points_maps;

  for (const auto& [stop_name, point] : stop_points) {
    points_maps.stops[stop_name] =
        {compressor.MapLon(point.longitude), compressor.MapLat(point.latitude)};
  }

  points_maps.companies.resize(companies.size());
  for (const auto& company : companies) {
    const auto point = company.address().coords();
    points_maps.companies[company.id()] =
        {compressor.MapLon(point.lon()), compressor.MapLat(point.lat())};
  }
...
MapRenderer::MapRenderer(const TransportCatalog& catalog)
    : catalog_(catalog)
{
}
...
MapRenderer::MapRenderer(const Descriptions::StopsDict& stops_dict,
    ...
    : catalog_(catalog),
      ...
      points_maps_(ComputeObjectsCoordsByGrid(stops_dict, buses_dict, catalog.GetCompanies(), render_settings_)),
...
void MapRenderer::Serialize(TCProto::MapRenderer& proto) {
  ...
  auto& points_maps_proto = *proto.mutable_points_maps();

  for (const auto& [name, point] : points_maps_.stops) {
    auto& stop_point_proto = *points_maps_proto.add_stops();
    stop_point_proto.set_name(name);
    Svg::SerializePoint(point, *stop_point_proto.mutable_point());
  }

  for (const auto point : points_maps_.companies) {
    auto& point_proto = *points_maps_proto.add_companies();
    Svg::SerializePoint(point, point_proto);
  }
...
std::unique_ptr<MapRenderer> MapRenderer::Deserialize(const TCProto::MapRenderer& proto, const TransportCatalog& catalog) {
  std::unique_ptr<MapRenderer> renderer_holder(new MapRenderer(catalog));
  auto& renderer = *renderer_holder;

  renderer.render_settings_ = RenderSettings::Deserialize(proto.render_settings());

  const auto& points_maps = proto.points_maps();
  for (const auto& stop_point_proto : points_maps.stops()) {
    renderer.points_maps_.stops.emplace(stop_point_proto.name(), Svg::DeserializePoint(stop_point_proto.point()));
  }
  renderer.points_maps_.companies.reserve(points_maps.companies_size());
  for (const auto point_proto : points_maps.companies()) {
    renderer.points_maps_.companies.push_back(Svg::DeserializePoint(point_proto));
  }
...
using RouteCompanyItem = TransportRouter::RouteInfo::WalkToCompanyItem;
using RouteWaitCompanyItem = TransportRouter::RouteInfo::WaitCompanyItem;
...
void MapRenderer::RenderBusLines(Svg::Document& svg) const {
  for (const auto& [bus_name, bus] : buses_dict_) {
    ...
    for (const auto& stop_name : stops) {
      line.AddPoint(points_maps_.stops.at(stop_name));
    }
    svg.Add(line);
  }
}
...
void MapRenderer::RenderBusLabel(Svg::Document& svg, const string& bus_name, const string& stop_name) const {
  ...
  const auto point = points_maps_.stops.at(stop_name);
...
void MapRenderer::RenderStopPoints(Svg::Document& svg) const {
  for (const auto& [_, point] : points_maps_.stops) {
...
void MapRenderer::RenderRouteStopPoints(Svg::Document& svg, const TransportRouter::RouteInfo& route) const {
  for (const auto& item : route.items) {
    ...
    for (size_t stop_idx = bus_item.start_stop_idx; stop_idx <= bus_item.finish_stop_idx; ++stop_idx) {
      const string& stop_name = stops[stop_idx];
      RenderStopPoint(svg, points_maps_.stops.at(stop_name));
    }
  }
}
...
void MapRenderer::RenderRouteStopLabels(Svg::Document& svg, const TransportRouter::RouteInfo& route) const {
  ...
  for (const auto& item : route.items) {
    ...
    RenderLabel(svg, points_maps_.stops.at(stop_name), stop_name);
  }

  auto last_item_it = route.items.crbegin();
  while (!holds_alternative<RouteBusItem>(*last_item_it) && !holds_alternative<RouteCompanyItem>(*last_item_it)) {
    ++last_item_it;
  }

  // draw stop label for last stop
  const string* last_stop_name = nullptr;
  if (holds_alternative<RouteBusItem>(*last_item_it)) {
    const auto& last_bus_item = get<RouteBusItem>(*last_item_it);
    last_stop_name = &buses_dict_.at(last_bus_item.bus_name).stops[last_bus_item.finish_stop_idx];
  } else {
    last_stop_name = &get<RouteCompanyItem>(*last_item_it).stop_name;
  }
  RenderLabel(svg, points_maps_.stops.at(*last_stop_name), *last_stop_name);
}

void MapRenderer::RenderCompanies(Svg::Document&) const {}

static const RouteCompanyItem* FindCompanyItem(const TransportRouter::RouteInfo& route_info) {
  if (route_info.items.empty()) {
    return nullptr;
  }
  auto last_item_it = route_info.items.crbegin();
  if (holds_alternative<RouteCompanyItem>(*last_item_it)) {
  } else if (holds_alternative<RouteWaitCompanyItem>(*last_item_it)) {
    ++last_item_it;
  } else {
    return nullptr;
  }
  return &get<RouteCompanyItem>(*last_item_it);
}

void MapRenderer::RenderRouteCompanyPoints(Svg::Document& svg, const TransportRouter::RouteInfo& route_info) const {
  const auto* company_item = FindCompanyItem(route_info);
  if (!company_item) {
    return;
  }

  const auto point = points_maps_.companies[company_item->company_id];
...
void MapRenderer::RenderRouteCompanyLabels(Svg::Document& svg, const TransportRouter::RouteInfo& route_info) const {
  const auto* company_item = FindCompanyItem(route_info);
  if (!company_item) {
    return;
  }
  ...
  RenderLabel(
      svg,
      point,
      YellowPages::PrintCompany(catalog_.GetCompanyById(company_item->company_id), catalog_.GetRubricsById())
  );
}

void MapRenderer::RenderRouteCompanyLines(Svg::Document& svg, const TransportRouter::RouteInfo& route_info) const {
  const auto* company_item = FindCompanyItem(route_info);
  if (!company_item) {
    return;
  }
  ...
  line.SetStrokeColor("black")
      ...
      .SetStrokeLineCap("round").SetStrokeLineJoin("round");
  line.AddPoint(points_maps_.stops.at(company_item->stop_name));
  line.AddPoint(points_maps_.companies[company_item->company_id]);
  svg.Add(line);
}
...
> MapRenderer::MAP_LAYER_ACTIONS = {
    ...
    {"company_lines",  &MapRenderer::RenderCompanies},
    {"company_points", &MapRenderer::RenderCompanies},
    {"company_labels", &MapRenderer::RenderCompanies},
};
